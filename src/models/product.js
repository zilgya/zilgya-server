...
const postProduct = async(body,image,id)=>{
    const {name,description,price,stock,categories_id} = body
    try {
        let queryParams = []
        let params = []
        const sqlQuery = 'INSERT INTO products(name,description,price,stock,categories_id,users_id) VALUES($1,$2,$3,$4,$5,$6) returning id'

        const product = await db.query(sqlQuery,[name,description,price,stock,categories_id,id])
