...
const getMyProducts = (id, query, route) => {
  return new Promise((resolve, reject) => {
    ...
    let arr = [id];
    let totalQuery =
      "select count(*) over() as total_products, * from (SELECT distinct on(p.id) p.id,p.name, i.url as image_url, p.description, p.price, p.stock, p.stock_condition, c.name as category, b.name as brand, c2.name as color, u.id as seller_id,p.created_at ,p.on_delete, p.updated_at  FROM products p join categories c on p.categories_id =c.id join users u on p.users_id = u.id join brands b on p.brands_id =b.id join colors c2 on p.colors_id =c2.id join images i on i.product_id=p.id where p.on_delete = false and p.users_id = $1) as p";
    let sqlQuery =
      "SELECT * from (SELECT distinct on(p.id) p.id,p.name, i.url as image_url, p.description, p.price, p.stock, p.stock_condition, c.name as category, b.name as brand, c2.name as color, u.id as seller_id,p.created_at ,p.on_delete, p.updated_at  FROM products p join categories c on p.categories_id =c.id join users u on p.users_id = u.id join brands b on p.brands_id =b.id join colors c2 on p.colors_id =c2.id join images i on i.product_id=p.id where p.on_delete = false and p.users_id = $1) as p";
    if (!find && !categories) {
